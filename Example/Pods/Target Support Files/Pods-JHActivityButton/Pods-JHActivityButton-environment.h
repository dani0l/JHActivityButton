
// To check if a library is compiled with CocoaPods you
// can use the `COCOAPODS` macro definition which is
// defined in the xcconfigs so it is available in
// headers also when they are imported in the client
// project.


// AHEasing
#define COCOAPODS_POD_AVAILABLE_AHEasing
#define COCOAPODS_VERSION_MAJOR_AHEasing 1
#define COCOAPODS_VERSION_MINOR_AHEasing 2
#define COCOAPODS_VERSION_PATCH_AHEasing 0

// JHActivityButton
#define COCOAPODS_POD_AVAILABLE_JHActivityButton
#define COCOAPODS_VERSION_MAJOR_JHActivityButton 1
#define COCOAPODS_VERSION_MINOR_JHActivityButton 0
#define COCOAPODS_VERSION_PATCH_JHActivityButton 0

// Masonry
#define COCOAPODS_POD_AVAILABLE_Masonry
#define COCOAPODS_VERSION_MAJOR_Masonry 0
#define COCOAPODS_VERSION_MINOR_Masonry 6
#define COCOAPODS_VERSION_PATCH_Masonry 0

